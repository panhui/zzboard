<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-100 relative overflow-hidden">
    ...
    <!-- Language Switcher (Copied from Dashboard) -->
    ...
    <div class="absolute -top-20 -right-20 w-96 h-96 bg-blue-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20"></div>
    <div class="absolute -bottom-20 -left-20 w-96 h-96 bg-purple-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20"></div>
    
    <el-card class="w-full max-w-md z-10 !rounded-2xl shadow-xl border-none">
      <template #header>
        <div class="text-center">
          <h2 class="text-2xl font-bold text-gray-800">{{ $t('forgot_password.title') }}</h2>
          <p class="text-sm text-gray-500 mt-2">{{ $t('forgot_password.subtitle') }}</p>
        </div>
      </template>
      ...
      <el-form :model="form" :rules="rules" ref="formRef" size="large">
        ...
        <el-form-item prop="password">
          <el-input v-model="form.password" type="password" :placeholder="$t('forgot_password.new_password')" :prefix-icon="Lock" show-password />
        </el-form-item>
        ...
        <el-button type="primary" class="w-full !rounded-xl !h-12 !text-lg !font-medium" :loading="loading" @click="handleSubmit">
          {{ $t('forgot_password.submit') }}
        </el-button>

        <div class="mt-6 text-center text-sm text-gray-500">
          <router-link to="/login" class="text-blue-500 font-medium hover:underline">{{ $t('forgot_password.back_login') }}</router-link>
        </div>
      </el-form>
    </el-card>
  </div>
...
import { forgetPassword, sendEmailVerify, getGuestConfig } from '../../api/auth';
...
const validatePass2 = (rule: any, value: any, callback: any) => {
  if (value === '') {
    callback(new Error(t('register.confirm_password')));
  ...
  await formRef.value.validate(async (valid: boolean) => {
    if (valid) {
      ...
      try {
        ...
        ElMessage.success(t('forgot_password.success'));
        router.push('/login');
      } catch(e) {
        console.error(e);
      } finally {
